...
- **Configurable Opportunity Statuses (July 2026):** `app_settings.opportunity_statuses` jsonb holds tenant-configurable status options (Settings > Field Options > Opportunities). Server validation in `PATCH /api/settings` requires unique values and the system statuses qualifying/won/lost (deletable protection also in `FieldOptionEditor` via `protectedValues`). Labels are renameable and options reorderable; consumed (with locale defaults fallback) in opportunities list/detail and the Opportunity Pipeline report. Open/won/lost semantics remain keyed to stable values — any non-won/non-lost status counts as open.
